...
pub struct SuspendFareCredentialCommand {
    /// Credential that should become temporarily unusable.
    pub credential_id: FareCredentialId,
}

/// Stable application result returned after credential suspension.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct SuspendedFareCredential {
...
impl SuspendedFareCredential {
    ...
    /// Returns the status observed before suspension.
    #[must_use]
    pub const fn previous_status(self) -> FareCredentialStatus {
        self.previous_status
    }

    /// Returns the authoritative status after suspension.
    #[must_use]
...
/// Coordinates fare-credential suspension.
///
...
pub struct SuspendFareCredentialService<'a> {
    transaction_manager: &'a dyn TransactionManager,
    clock: &'a dyn Clock,
    event_id_generator: &'a dyn DomainEventIdGenerator,
}

impl<'a> SuspendFareCredentialService<'a> {
    /// Creates the suspension service from application ports.
    #[must_use]
    pub const fn new(
        transaction_manager: &'a dyn TransactionManager,
        clock: &'a dyn Clock,
    ...
    /// Suspends an active fare credential.
    ///
    /// Suspending an already suspended credential is idempotent and creates
    /// no duplicate status-change event.
    pub async fn execute(
        &self,
        command: SuspendFareCredentialCommand,
    ) -> Result<SuspendedFareCredential, ApplicationError> {
        ...
        if previous_status == FareCredentialStatus::Suspended {
            transaction.commit().await?;

            return Ok(SuspendedFareCredential {
                credential_id: command.credential_id,
                previous_status,
                current_status: FareCredentialStatus::Suspended,
                changed: false,
            });
        }
        ...
        let event = match DomainEvent::new(
            ...
            Err(_) => {
                let error = ApplicationError::conflict(
                    "suspend fare credential",
                    "credential suspension event is invalid",
                );
...
mod tests {
    ...
    fn execute(
        ...
        let service = SuspendFareCredentialService::new(&manager, &clock, &event_ids);
    ...
    #[test]
    fn active_credential_is_suspended_atomically() {
        ...
        let result = execute(
            Arc::clone(&state),
            SuspendFareCredentialCommand { credential_id },
            event_id,
        );
        ...
        assert!(matches!(
            result,
            Ok(suspended)
                if suspended.credential_id()
                    == credential_id
                    && suspended.previous_status()
                        == FareCredentialStatus::Active
                    && suspended.current_status()
    ...
    #[test]
    fn event_append_failure_discards_staged_suspension() {
        let credential_id = FareCredentialId::generate();
