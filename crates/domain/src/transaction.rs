...
pub enum FareTransactionValueError {
    ...
    #[error(
        "event time cannot be negative: {unix_milliseconds}"
    )]
    NegativeEventTime {
...
impl EventTime {
    ...
    pub const fn from_unix_milliseconds(
        ...
        if unix_milliseconds < 0 {
            return Err(
                FareTransactionValueError::NegativeEventTime {
                    unix_milliseconds,
                },
            );
        }
...
pub enum FareDecisionError {
    ...
    #[error(
        "approved fare amount cannot be negative: {amount}"
    )]
...
impl FareDecision {
    ...
    pub fn approved(
        ...
        if charged_amount.is_negative() {
            return Err(
                FareDecisionError::NegativeApprovedAmount {
...
pub enum FareTransactionError {
    ...
    #[error(
        "fare transaction decision conflict: existing {existing:?}, requested {requested:?}"
    )]
    DecisionConflict {
    ...
    #[error(
        "fare transaction cannot be reconciled while its status is {status}"
    )]
...
impl FareTransaction {
    ...
    #[must_use]
    pub const fn transit_account_id(
        &self,
    ) -> TransitAccountId {
    ...
    #[must_use]
    pub const fn fare_credential_id(
        &self,
    ) -> FareCredentialId {
    ...
    #[must_use]
    pub const fn local_sequence_number(
        &self,
    ) -> LocalSequenceNumber {
    ...
    #[must_use]
    pub const fn fare_policy_version(
        &self,
    ) -> FarePolicyVersion {
    ...
    pub fn mark_reconciled(
        ...
        match self.status {
            ...
            FareTransactionStatus::Decided => {
                self.status =
                    FareTransactionStatus::Reconciled;
                Ok(())
            }
...
mod tests {
    ...
    fn valid_event_time() -> EventTime {
        match EventTime::from_unix_milliseconds(
            1_700_000_000_000,
        ) {
            Ok(value) => value,
            Err(error) => {
    ...
    fn new_transaction_is_pending_decision() {
        let transaction =
            pending_transaction(FareProcessingMode::Online);
    ...
    fn approved_decision_moves_transaction_to_decided() {
        ...
        let result =
            transaction.record_decision(decision);
    ...
    fn rejected_decision_records_no_charge() {
        let mut transaction =
            pending_transaction(FareProcessingMode::Online);
        let decision = FareDecision::rejected(
            FareRejectionReason::CredentialRevoked,
        );
    ...
    fn conflicting_decision_is_rejected() {
        ...
        let requested = FareDecision::rejected(
            FareRejectionReason::ProductInvalid,
        );
        ...
        let result =
            transaction.record_decision(requested);
    ...
    fn decided_transaction_can_be_reconciled() {
        ...
        assert!(
            transaction
                .record_decision(approved_decision(250))
                .is_ok()
        );
    ...
    fn offline_processing_mode_is_preserved() {
        let transaction =
            pending_transaction(FareProcessingMode::Offline);
