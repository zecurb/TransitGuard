...
use transitguard_fare_engine::{
    ...
    TransferHistory, TransferWindow, ZoneId, evaluate_fare, evaluate_fare_offline,
};
...
fn input(balance_minor_units: i64) -> FareEvaluationInput {
    FareEvaluationInput {
        ...
        available_balance: Money::from_minor_units(balance_minor_units, Currency::Usd),
    }
}

fn offline_context(provisional_limit_minor_units: i64) -> OfflineEvaluationContext {
    OfflineEvaluationContext {
        ...
        provisional_charge_limit: Money::from_minor_units(
...
fn online_and_offline_modes_share_calculation_evidence() {
    ...
    let offline_result = evaluate_fare_offline(policy, input, offline_context(1_000));
    ...
    assert_eq!(online.evidence(), offline_evaluation.evidence());

    assert_eq!(online.policy_id(), offline_evaluation.policy_id());

    assert_eq!(online.policy_version(), offline_evaluation.policy_version());

    assert_eq!(online.event_time(), offline_evaluation.event_time());

    assert_eq!(
        online.outcome(),
        FareEvaluationOutcome::Approved {
            charged_amount: Money::from_minor_units(200, Currency::Usd,),
...
fn offline_mode_preserves_online_insufficient_balance_rejection() {
    ...
    assert_eq!(offline.evaluation().outcome(), online.outcome());

    assert_eq!(offline.evaluation().evidence(), online.evidence());
}
...
fn offline_risk_rules_do_not_change_calculated_fare() {
    ...
    let offline_result = evaluate_fare_offline(policy, input, offline_context(199));
...
fn repeated_online_and_offline_evaluations_are_identical() {
    ...
    assert_eq!(evaluate_fare(policy, input), evaluate_fare(policy, input));
